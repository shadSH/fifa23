...
class GenerateControllerCommand extends GeneratorCommand
{
    ...
    protected function getStub()
    {
        ...
        ? config('technobase_generator.path').'/controller.stub'
    ...
    protected function buildClass($name, $request = 'request')
    {
        ...
        $new_model = '';
        $modelName_smaller = snake_case($this->option('crud-name'), '_');
        if (count($model_crud) > 0) {
            foreach ($model_crud as $crud) {
                $new_model .= "$$modelName_smaller->$crud = $$request->$crud;\n";
            }
        }
        ...
        $modelName_smaller = snake_case($this->option('model-name'), '_');
        ...
        $viewName = snake_case($this->option('crud-name'), '_');
...
EOD;
        ...
        return $this->replaceNamespace($stub, $name)
            ...
            ->replaceWhereNewRouteName($stub, $viewName)
    ...
    protected function replaceWhereNewRouteName(&$stub, $viewName)
    {
        $stub = str_replace(
            '{{routeName}}', $viewName, $stub
        );
